  ...
  static GoRouter createRouter(BuildContext context) {
    ...
    return GoRouter(
      ...
      redirect: (context, state) {
        ...
        // Use AdminService for admin checks
        final isAdmin =
  ...
  static StatefulShellRoute _createShellRoute() {
    return StatefulShellRoute.indexedStack(
      builder: (context, state, navigationShell) {
        ...
        DebugLogger.info(
            'Shell Route - Current index: ${navigationShell.currentIndex}, Route: ${state.uri.path}');

        // Use a custom shell layout that manages navigation based on user role
        return Scaffold(
          ...
          bottomNavigationBar: Consumer<AuthProvider>(
            builder: (context, authProvider, _) {
              ...
              // REMOVED notifications icon from navbar for both admin and regular users
              final List<NavigationDestination> destinations = [
                ...
                // Removed Notifications item from navbar for all users
              ];

              // Safe index calculation to prevent out-of-bounds errors
              ...
              return NavigationBar(
                ...
                onDestinationSelected: (index) {
                  // Get current route for logging purposes
                  final currentRoute = state.uri.path;
                  String targetRoute;
                  String tabName;

                  // Determine target route based on index and user role
                  if (index == 0) {
                    targetRoute = '/home';
                    tabName = 'Home';
                  } else if (index == 1) {
                    ...
                    if (isAdmin) {
                      ...
                      // No index 4 for non-admin users after removing notifications
                      targetRoute = '/home';
                      tabName = 'Home';
                    }
                  } else {
                    targetRoute = '/home'; // Default fallback
                    tabName = 'Home';
                  }

                  // Log the navigation click with details
